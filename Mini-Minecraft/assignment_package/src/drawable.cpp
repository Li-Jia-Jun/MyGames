...
TransparentDrawable::TransparentDrawable(OpenGLContext* mp_context)
    : Drawable(mp_context), m_TransCount(-1), m_TransBufIdx(-1), m_TransIdxGenerated(false)
{}

void TransparentDrawable::destroyVBOdata()
{
...
int TransparentDrawable::transparentElementCount()
{
    return m_TransCount;
}

void TransparentDrawable::generateTransparentIdx()
{
    m_TransIdxGenerated = true;
    mp_context->glGenBuffers(1, &m_TransBufIdx);
}

bool TransparentDrawable::bindTransIdx()
{
...
void Drawable::setCount(int c)
{
    this->m_count = c;
}

void TransparentDrawable::setTransCount(int c)
{
    this->m_TransCount = c;
}
